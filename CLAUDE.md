...
- `js/interpreter/path-offset.js` — **nucleo comune della compensazione del raggio** (tornio e fresa): spostamento del percorso nel piano u–v, spigoli, attivazione e annullamento. Ogni macchina passa degli agganci (`toElement`, `startCenter`, `endCenter`, `toMove`): la geometria va corretta qui una volta sola. `cutter-compensation.js` sono gli agganci della fresa (u = X, v = Y, Z preso dal movimento di origine; i movimenti solo in Z restano nel punto compensato).
- `js/interpreter/nose-compensation.js` — agganci del tornio per G41/G42: l'interprete segna i movimenti con `comp` e `rn`; dopo l'interpretazione `compensateNoseRadius()` sostituisce le catene di movimenti compensati con quelli della punta teorica (spigoli esterni con arco di raggio rn, interni accorciati al punto d'incontro, attivazione e annullamento come Fanuc). I movimenti di G71 non si compensano. Il tempo ciclo si calcola dopo la compensazione.
...
- `js/interpreter/interpret-mill.js` e `path-3d.js` — interprete della fresa (posizioni `{ x, y, z }`; archi nei piani G17/G18/G19 con `move.axes` = [primo, secondo, perpendicolare] e centro con le quote dei due assi del piano, anche elicoidali; G41/G42 con D uguale all'utensile (2010), solo in G17; cicli G81/G82/G83 con G98/G99). `js/machines/mill/` — `codes.js`, `machine.js` (grezzo in quote pezzo con `stockBox()`, riga `(GREZZO X.. Y.. Z..)`), `tools.js` (forma del fondo con `bottomAt()`), `stock.js` mappa delle altezze, `simulator.js` (asportazione, 3005, 3008, 4001, 4003 gambo e portautensile, 4004 morsa; `runAllMill()` per i test).
...
- **Database delle macchine**: `js/machines/catalog-data.js` contiene solo dati (un oggetto per modello, con `sourceUrl`, `checked`, `iso`); `js/machines/catalog.js` converte un modello nei parametri del simulatore (`latheModelData`, `millModelData`) con ipotesi scritte nel codice. Gli adattatori sono fabbriche: `createLatheAdapter(modello)`, `createMillAdapter(modello)`; `latheAdapter`/`millAdapter` sono quelli della macchina generica. **Mai inventare dati di una macchina**: se un valore non è nelle fonti ufficiali resta `null`. Il linguaggio resta Fanuc ISO per tutti i modelli; un eventuale dialetto Siemens sarebbe un nuovo interprete dietro lo stesso adattatore, non una modifica dei simulatori.
- **Linguaggio Siemens**: `js/parser/parse-siemens.js` legge la sintassi (nomi con `=`, `IC()`/`AC()`, parole chiave, chiamate `CYCLE…`); le tabelle `codes-siemens.js` hanno `parseProgram`, `keywords` e `planned` (parole non simulate → 1013); `js/interpreter/siemens-to-fanuc.js` traduce ogni blocco nei blocchi Fanuc equivalenti con la stessa riga (`block.original` = blocco Siemens) e poi lavora l'interprete Fanuc. Gli adattatori ricevono il linguaggio (`createLatheAdapter(modello, 'siemens')`), espongono `check(text)` e riscrivono i testi degli allarmi con `js/alarms/siemens-hints.js`. Non duplicare logica di macchina per il Siemens: se una funzione Siemens ha un equivalente Fanuc, va tradotta. I cicli di foratura (`CYCLE81`/`82`/`83`, `MCALL` in `state.mcall`) diventano `G99 G8x`, `G80` e un rapido al piano RTP. `CYCLE95` diventa `G71`/`G70`: il profilo sta dopo `M30` tra due etichette (`block.label`), riceve due N propri (900000…) e il primo punto si divide in `G0 X` + `Z`; il blocco `G71` tradotto ha `profileElsewhere`, così l'interprete prosegue con il blocco dopo il ciclo invece di saltare dopo Q. `CYCLE93` diventa una serie di `G0`/`G1`/`G4` con la riga del ciclo: serve la larghezza del troncatore, quindi `translateSiemens(blocks, macchina, { tools })` riceve la tabella utensili. `POCKET3`/`POCKET4` stanno in `js/interpreter/siemens-pockets.js`: percorso del centro fresa (anelli concentrici, piani di MID, elica facoltativa) in `G0`/`G1`/`G2`/`G3` con la riga del ciclo; con `G41`/`G42` attivo (`state.comp`) danno 1013. `CYCLE71` sta in `js/interpreter/siemens-facing.js` (passate `G1` del centro fresa) e segue le stesse regole. Gli esempi hanno `"dialect": "siemens"` in `examples/index.json`; il test di equivalenza confronta i pezzi prodotti dai due linguaggi.
