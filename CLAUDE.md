...
- **Linguaggio Siemens**: `js/parser/parse-siemens.js` legge la sintassi (nomi con `=`, `IC()`/`AC()`, parole chiave, chiamate `CYCLE…`); le tabelle `codes-siemens.js` hanno `parseProgram`, `keywords` e `planned` (parole non simulate → 1013); `js/interpreter/siemens-to-fanuc.js` traduce ogni blocco nei blocchi Fanuc equivalenti con la stessa riga (`block.original` = blocco Siemens) e poi lavora l'interprete Fanuc. Gli adattatori ricevono il linguaggio (`createLatheAdapter(modello, 'siemens')`), espongono `check(text)` e riscrivono i testi degli allarmi con `js/alarms/siemens-hints.js`. Non duplicare logica di macchina per il Siemens: se una funzione Siemens ha un equivalente Fanuc, va tradotta. Gli esempi hanno `"dialect": "siemens"` in `examples/index.json`; il test di equivalenza confronta i pezzi prodotti dai due linguaggi.
